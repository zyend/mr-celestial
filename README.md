# Moonrise Celestial Signs Calculator v6
...
Version 1.0.6 adds the Rising Sign / Ascendant and places it after Moon and before Mercury:
...
The ten planetary bodies still use Swiss Ephemeris `calc_ut()` tropical ecliptic longitude. Rising uses Swiss Ephemeris `houses()` / `swe_houses()` and reads `ascmc[0]`, the Ascendant longitude.

Example Rising row:

```text
Rising | ♑ Capricorn | Your Ascendant was at 5° 32′ 16″ Capricorn at the time of your birth.
                       Explore Capricorn healing crystals →
```
...
6. The Sun, Moon, Mercury, Venus, Mars, Jupiter, Saturn, Uranus, Neptune, and Pluto are calculated for the exact Julian day.
7. Swiss Ephemeris calculates the Ascendant from the same Julian day plus the selected city's latitude and longitude.
8. Each tropical longitude is converted to its zodiac sign and degree/minute/second position.
9. Each result links to Moonrise's `/zodiac/{sign}-healing-crystals/` page.
...
The v6 local database now retains the source dataset's coordinates in addition to timezone:
...
When upgrading from v5, v6 recognizes the old timezone-only database as outdated and rebuilds it with coordinates. If needed, repair it under:
...
The v6 settings-page browser test verifies:
...
1. Upload the v6 ZIP through **Plugins → Add New → Upload Plugin** and replace the existing plugin.
